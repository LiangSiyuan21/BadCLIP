...
def get_loss(umodel, outputs, criterion, options, gather_backdoor_indices, pos_embeds=None, neg_embeds=None):  
    ...
    if 'pos' in options.name:
        ...
        if 'neg' in options.name:
            ...
            loss['triplet'] = criterion_tri(image_embeds,  selected_pos_embeds.to(options.device),  selected_neg_embeds.to(options.device)) *10
        else:
            # 使用这些索引来获取数据
            loss['near_pos'] = criterion_MSE(image_embeds, selected_pos_embeds.to(options.device)) * 10
...
def optimize_patch(rank, options, logger):
    ...
    if options.init == 'const':
        ...
        rand_patch = np.random.normal(loc=0.5, scale=0.25, size=[1, 3, options.res, options.res])
